...
/*
	...
	OPTIONS (default values in brackets):
		...
		showsurveys (true) - display the survey boundaries
...
function VirtualSky(input){
	...
	this.showsurveys = true; 			// Display the survey boundaries
	this.meteorshowers = false;			// Display meteor shower radiants
	this.negative = false;				// Invert colours to make it better for printing
	this.showgalaxy = { lines: false, labels: false }; // Display the Milky Way
	this.showstars = false;				// Display current positions of the stars
	...
	this.ground = false;
	this.grid = { az: false, eq: true, gal: false, step: 30 };	// Display grids
...
// 		galaxy: this.dir+"galaxy.json",          // Data for milky way - 12 kB
		surveys: this.dir+"galaxy.json",		  // Data for survey boundaries
...
VirtualSky.prototype.init = function(d){
	...
	if(is(d.galaxylabels,b)) this.galaxy.labels = d.galaxylabels;
	if(is(d.surveys,b)) this.surveys = d.surveys;
...
VirtualSky.prototype.getPhrase = function(key,key2){
	...
	if(key==="constellations"){
		if(key2 && this.lang.constellations && is(this.lang.constellations[key2], "string"))
...
// SOMEHOW COME UP WITH A FLAG MECHANISM TO BE ABLE TO TOGGLE WITH FULL SCREEN AND NOT
// Currently using a global variable. Not the best idea, but also not the worst. Perhaps come up with a button that stays 
// 		clicked and when you unclick it, the screen goes back to normal.
// Also have to move the buttons off the canvas for when the screen gets larger
// Do we like a resize button? 	

VirtualSky.prototype.fullScreenSet = function(w,h) {
	if(fullScreenCount%2 == 1){	
		this.resize(window.innerWidth, window.innerHeight-300)
	} else {
		this.resize(window.innerWidth, window.innerHeight-10)
	}
	fullScreenCount++;
};
...
VirtualSky.prototype.setFOV = function(fov){
	if(fov > 100 || typeof fov!=="number") this.fov = 100;
...
// loads in the json file and sends an error if the file was not correctly loaded
...
VirtualSky.prototype.createSky = function(){
	...
	// get the survey data
	if(!this.surveys) {
		Parse.initialize("Q3dS7g4tWskpgwfghavNvT09shW7iQxVBiZjbmjb", "e35fAfNBztSItS11qWl1B6uYDxwmxvFBoiOCUhir");
		var self = this;
		var SurveyObj = Parse.Object.extend("SurveyObj");
		var queryObject = new Parse.Query(SurveyObj);
		tempArray = [];
		...
		queryObject.find({
		    success: function (results) {
		        for (var i = 0; i < results.length; i++) {
								tempArray[i] = {};
		            tempArray[i]["name"] = results[i].get("name");
								tempArray[i]["col"] = results[i].get("col");
								tempArray[i]["coords"] = results[i].get("coords");
								tempArray[i]["attribs"] = results[i].get("attribs");
							  tempArray[i]["contact"] = results[i].get("contact");
							  tempArray[i]["img"] = results[i].get("img");
							  tempArray[i]["otherInfo"] = results[i].get("otherInfo");

							  $("#surveyOpts").append($("<option />").val(tempArray[i]["coords"].slice(0,2)).text(tempArray[i]["name"]));
		        }

						// Array with all surverys - self.surveys[i].attribs.Area/Band/etc. (i being index of survey)
						// Arrtibs - {"Area":"","band":"19.2","date":"","expTime":"","lMag":"","psf":"","telescope":""}
	...
	this.registerKey('v',function(){ this.toggleGalaxyLabels(); },'names');
...
VirtualSky.prototype.drawPlanets = function(){
	...
	var colour = this.col.grey; // initializing planet color
...
// surveys[i] refers to each survey from surveydata.json
// surveys[1][i] refers to the index of each part of the structure for each survey
//
// VirtualSky.prototype.drawSurveys = function(){
// 	if(!this.surveys || this.surveys.length <= 0) return this;
// 	var i;
// 	var colour = this.col.red;
//
// 	for(i = 0; i < this.surveys.length; i++){
// 		// first check to see if a next planet can be made
// 		if(!this.surveys[i]) continue;
// 		// setting color from the .json file
// 		colour = this.surveys[i][1];
// 		if(!this.showsurveys){
// 			drawSurvey(colour, this.surveys[i][0]);
// 		}
// 	}
//
// 	return this;
...
VirtualSky.prototype.drawSurvey = function(colour,label){
	...
	for(c = 0; c < this.surveys.length; c++){
		...
		this.ctx.beginPath();
		//set colors here
		this.ctx.strokeStyle = this.surveys[c]['col'];
		this.ctx.fillStyle = this.surveys[c]['col'];
		//
		// console.log("hi");

		// Get a copy of the current shape
		p = this.surveys[c]['coords'];
...
VirtualSky.prototype.toggleSurveys = function(){ this.surveys = !this.surveys; this.draw(); return this; }
VirtualSky.prototype.toggleGalaxyLabels = function(){ this.galaxy.labels = !this.galaxy.labels; this.draw(); return this; }
...
VirtualSky.prototype.addPointer = function(input){
	...
	if(matched < 0){
		...
		if(typeof p.html !== "string"){
			...
			credit = p.credit || "Metadata";

			if (p.type == 'survey') {
				// p.html = '<div class="virtualsky_infocredit">' + 'Hello World' + '</div>';
				// p.htlm = '<button type="button" class="btn btn-info" data-container="body" data-toggle="popover" data-placement="top" data-content=planetarium.surveys[0].attribs.Area>'+'Popover Example'+'</button>
				metadata = "Metadata 1"
				p.html = '<a id="popover" data-trigger="hover">'+ metadata +'</a>'
			...
			// <button type="button" class="btn btn-info" data-container="body" data-toggle="popover" data-placement="top" data-content=planetarium.surveys[0].attribs.Area> Popover Example </button>
